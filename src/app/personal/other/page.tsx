...
export default function OtherInput() {
  ...
  const go = () => {
    const t = text.trim()
    if (!t) return
    router.push(`/status?category=${category}&subtype=${encodeURIComponent(t)}`)
  }
    ...
    <GeoGate>
      <div className="min-h-[100svh] bg-white flex flex-col">
        ...
        <main className="flex-1 max-w-md mx-auto p-6 space-y-4">
          ...
          <p className="text-sm text-gray-700">Kısaca durumu yazın (ör. “göğüs ağrısı”, “gaz kaçağı”).</p>
          <textarea
            value={text} onChange={e=>setText(e.target.value)} rows={4}
            className="w-full border rounded-xl p-3"
            placeholder="Durumunuz…"
          />
          <button onClick={go} className="w-full bg-[#D32F2F] text-white rounded-lg py-2 font-semibold">
            Devam et
          </button>
        </main>
        <BottomNav />
