...
export function addSimIncident(
  i: Omit<SimIncident, 'id' | 'ts'> & { ts?: number }
) {
  const item: SimIncident = {
    id: `sim-${Date.now()}`,
    ts: i.ts ?? Date.now(),
    ...i,
  }
...
export function removeSimIncident(id: string) {
  try {
    const raw = localStorage.getItem('nearby_simulated')
    if (!raw) return
    const arr: SimIncident[] = JSON.parse(raw)
    const filtered = arr.filter((x) => x.id !== id)
    localStorage.setItem('nearby_simulated', JSON.stringify(filtered))
  } catch {}
}
