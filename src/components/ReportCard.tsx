...
import { Medical, Fire, Car, CloudRain, AlertTriangle, Users, MapPin, Calendar, HeartPulse } from "lucide-react";

const reportIcons = {
  'Medical': <Medical className="w-6 h-6 text-blue-500" />,
  'Fire': <Fire className="w-6 h-6 text-red-500" />,
  'Accident': <Car className="w-6 h-6 text-yellow-500" />,
  'Natural Disaster': <CloudRain className="w-6 h-6 text-green-500" />,
  ...
  return (
    <Card className="flex flex-col h-full shadow-md hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="flex-grow">
        <p className="text-muted-foreground line-clamp-2">{report.notes}</p>
      </CardContent>
